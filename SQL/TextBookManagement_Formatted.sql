...
CREATE TABLE DeliveryOfCargoFromStorageRecord
/*领用记录*/
(
...
ALTER TABLE
	OrderForm
ADD
	CONSTRAINT pk_orderForm FOREIGN KEY(ISBN) REFERENCES Book(ISBN);
...
ALTER TABLE
	DeliveryOfCargoFromStorageRecord
ADD
	CONSTRAINT fk_outStor FOREIGN KEY(ISBN) REFERENCES Book(ISBN);
...
INSERT INTO
	Administrator(
		adminID,
		adminPwd,
		adminModificationPermission,
		adminRemovePermission,
		adminPhoneNum
	)
VALUES
('admin', '6f67c16e74d856b6', 1, 1, '12580');
...
VALUES
(@date1, '4444444444444', 200, '跑路出版社', 2048),
...
FROM
	OrderForm
	JOIN Book ON OrderForm.ISBN = Book.ISBN;

INSERT INTO
	bePutInStorageRecord(
		TimeOfbePutInStorage,
		ISBN,
		bePutInStorageQuantity,
		storageRackNum
	)
VALUES
(GETDATE(), '4444444444444', 200, 'A'),
...
FROM
	bePutInStorageRecord
	JOIN Book ON bePutInStorageRecord.ISBN = Book.ISBN;

INSERT INTO
	DeliveryOfCargoFromStorageRecord(
		TimeOfDeliveryOfCargoFromStorage,
		ISBN,
		storageRackNum,
		DeliveryOfCargoFromStorageQuantity,
		getClass
	)
VALUES
(
...
FROM
	DeliveryOfCargoFromStorageRecord
	JOIN Book ON DeliveryOfCargoFromStorageRecord.ISBN = Book.ISBN;

/*存储过程*/
/*EXECUTE sp_help QueryBookRecord;  /*查询存储过程信息*/
DROP PROCEDURE QueryBookRecord;

* /
/*删除存储过程*/
/*DROP PROCEDURE QueryOrderFormbyType;*/
/*删除存储过程*/
/*CREATE PROCEDURE QueryOrderFormbyType			/*查询某种书籍的全部记录*/
@type varchar(50) AS BEGIN
...
FROM
	OrderForm
	JOIN Book ON Book.ISBN = OrderForm.ISBN
WHERE
	Book.bookType = @type;

END * / EXECUTE QueryOrderFormbyType @type = '社会科学';
...
@name varchar(50) AS BEGIN DECLARE @count1 int,
@count2 int,
@count3 int,
@count int;

SELECT
	@count1 = SUM(orderQuantity)
FROM
	OrderForm
	JOIN Book ON OrderForm.ISBN = Book.ISBN
WHERE
...
SELECT
	@count2 = SUM(bePutInStorageQuantity)
FROM
	bePutInStorageRecord
	JOIN Book ON bePutInStorageRecord.ISBN = Book.ISBN
WHERE
...
SELECT
	@count3 = SUM(DeliveryOfCargoFromStorageQuantity)
FROM
	DeliveryOfCargoFromStorageRecord
	JOIN Book ON DeliveryOfCargoFromStorageRecord.ISBN = Book.ISBN
WHERE
...
END * / EXECUTE CountRecord '盲僧技能分析与实例详解';
...
ON Book FOR DELETE AS BEGIN DELETE OrderForm
WHERE
	ISBN IN (
		SELECT
			ISBN
...
ON bePutInStorageRecord FOR
INSERT
	AS BEGIN DECLARE @quantity int;

SELECT
	@quantity = Book.bookInventoryQuantity
FROM
	Book
WHERE
	ISBN =(
...
SELECT
	@outQuan = bePutInStorageQuantity
FROM
	inserted
UPDATE
	Book
SET
	bookInventoryQuantity = @quantity + @outQuan
WHERE
	Book.ISBN =(
		SELECT
			ISBN
...
ON DeliveryOfCargoFromStorageRecord FOR
INSERT
	AS BEGIN DECLARE @quantity int;

SELECT
	@quantity = Book.demandedQuantity
FROM
	Book
WHERE
	ISBN =(
...
SELECT
	@outQuan = DeliveryOfCargoFromStorageQuantity
FROM
	inserted
UPDATE
	Book
